...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        # Умножение на weight выполняется ДО конвертации обратно в fp16
...
def apply_rotary_emb(xq, xk, freqs_cis):
    assert xq.shape[-1] % 2 == 0, f"head_dim must be even, got {xq.shape[-1]}"
...
class FeedForward(nn.Module):
    def __init__(self, dim, use_fused_silu=True):
        ...
        self.use_fused_silu = use_fused_silu
...
class SparseMoE(nn.Module):
    ...
    def forward(self, x):
        ...
        # Оптимизированная векторизованная маршрутизация
        ...
        if self.training:
            f_i = F.one_hot(topk_indices[:, 0], self.num_experts).float().mean(dim=0)
    ...
    def forward(self, x, freqs_cis, past_key_value=None, use_cache=False, x_norm_input=None, layer_idx=0):
        ...
        if past_key_value is not None and use_cache:
            ...
            if self.xquant_cache is not None and self.xquant_cache.mode != 'full':
                k, v = self.xquant_cache.update(
                    layer_idx, start_pos, T, k, v,
                    x_norm=x_norm_input, wk=self.wk.weight, wv=self.wv.weight
                )
...
class Block(nn.Module):
    def __init__(self, n_embd, n_head, n_kv_head=None, num_experts=8, num_shared_experts=0,
                 use_expert_choice=False, use_xquant=False):
        super().__init__()
        self.attention = MultiheadSelfAttention(n_embd, n_head, n_kv_head, use_xquant=use_xquant)
        self.feed_forward = SparseMoE(n_embd, num_experts=num_experts,
    ...
    def forward(self, x, freqs_cis, past_key_value=None, use_cache=False, layer_idx=0, x_norm_for_xquant=None):
        h, past_kv = self.attention(
            self.attention_norm(x), freqs_cis,
            past_key_value=past_key_value, use_cache=use_cache,
            x_norm_input=x_norm_for_xquant, layer_idx=layer_idx
        )
        x = x + h
