...
// ----------------------------------------------------
// 8. DATASET TUTORIAL DOCUMENT
// ----------------------------------------------------
function DatasetTutorialDoc() {
  return (
    <article className="docs-article">
      <div className="docs-article-header">
        <h1>Туториал 1: Искусство создания датасета</h1>
        <p className="docs-lead-paragraph">
          В мире LLM качество ваших данных определяет "потолок" возможностей вашей модели. 
          Этот гайд поможет вам собрать идеальный корпус для обучения.
        </p>
      </div>

      <hr className="docs-divider" />

      <h2>🏗️ Стратегия сбора данных</h2>
      <p>
        Сбор данных — это не просто скачивание текстов. Это архитектурный процесс. 
        Мы рекомендуем придерживаться <strong>правила 70/20/10</strong>:
      </p>

      <ul>
        <li><strong>70% "Фундамент":</strong> Википедия, новости, классическая литература. Это учит модель грамматике и базе.</li>
        <li><strong>20% "Индивидуальность":</strong> Ваши личные тексты, логи чатов, специализированная документация.</li>
        <li><strong>10% "Логика и Разнообразие":</strong> Код, стихи, математические задачи, форумы.</li>
      </ul>

      <AlertBox type="tip" title="Золотое правило">
        Лучше 100 МБ идеально вычищенного текста, чем 1 ГБ "шума".
      </AlertBox>

      <hr className="docs-divider" />

      <h2>🛠️ Пайплайн очистки</h2>
      <p>При импорте через Tolstoy-CLI (Меню [1]), текст проходит несколько этапов:</p>
      <ol>
        <li><strong>Удаление Unicode-мусора:</strong> Очистка невидимых управляющих символов.</li>
        <li><strong>Нормализация пробелов:</strong> Превращение множественных пробелов и табов в одиночные.</li>
        <li><strong>Дедупликация:</strong> Удаление повторяющихся кусков текста, чтобы модель не "зацикливалась".</li>
      </ol>

      <h3>Поддерживаемые форматы:</h3>
      <p>CLI автоматически извлекает текст из: <code>.txt, .md, .pdf, .docx, .json, .csv</code>.</p>
    </article>
  );
}
